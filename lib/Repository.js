...
class Repository extends Requestable {
  ...
  /**
   ...
   /**
    * Replace all the topics
    * @see https://developer.github.com/v3/repos/#replace-all-topics-for-a-repository
    * @param {Object} options - the new list of topics
    * @param {Requestable.callback} cb - will receive the list of created topics
    * @return {Promise} - the promise for the http request
    */
   replaceTopics(options, cb) {
      options = options || {};
      options.AcceptHeader = 'mercy-preview';
      return this._request('PUT', `/repos/${this.__fullname}/topics`, options, cb);
   }
    ...
    */
   getTopics(cb) {
      return this._request('GET', '/repos/' + this.__fullname + '/topics', {AcceptHeader: 'mercy-preview'}, cb);
   }

   /**
    * Get all the topics
    * @param {Object} number - the description of the param
    * @param {Object} options - the description of the param
    * @param {Requestable.callback} cb - will receive the list of created topics
    * @return {Promise} - the promise for the http request
    */
   assignPullRequest(number, options, cb) {
      options = options || {};
      options.AcceptHeader = 'symmetra-preview';
    ...
    * @return {Promise} - the promise for the http request
    */
   setPullRequestReview(number, options, cb) {
      return this._request('POST', '/repos/' + this.__fullname + '/pulls/' + number + '/reviews', options, cb);
   }

   /**
    * Get all the topics
    * @param {Object} branch - the description of the param
    * @param {Object} options - the description of the param
    * @param {Requestable.callback} cb - will receive the list of created topics
    * @return {Promise} - the promise for the http request
    */
   protectBranches(branch, options, cb) {
      options = options || {};
      options.AcceptHeader = 'luke-cage-preview';
      return this._request('PUT', '/repos/' + this.__fullname + '/branches/' + branch + '/protection', options, cb);
   }
}
